...
client.on('ready', () => {
  ...
  // set its activity. Will currently see as "Playing God" (Not anymore, that was a mistake)- Flame
  client.user.setActivity("Peasant")
...
function processCommand(receivedMessage) {
  ...
  if (primaryCommand == "help") {
    ...
    quoteCommand(arguments, recievedMessage)
...
function helpCommand(arguments, receivedMessage) {
  if (arguments.length == 0) {
    ...
    receivedMessage.channel.send("The commands you can use so far are `!mult, !add, !size, !roll (to roll a D20), !joke and !quote (which will most likely not work)`")
...
function quoteCommand(arguments, recievedMessage){
  receivedMessage.channel.send(`${quoteRando[Math.floor(Math.random() * quoteRando.length)].quote}`)
}
